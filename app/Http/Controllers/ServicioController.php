...
class ServicioController extends Controller
{
    ...
    {
        $tipo_servicio = DB::select("SELECT id, descripcion as nombre from tbl_servicio where deleted_at is null order by nombre");
    ...
    public function guardar_servicio(Request $request)
    {
        ...
        try {
            ...
            if ($accion == 1) {
                $nuevo_servicio = collect(\DB::select("INSERT into tbl_servicio (descripcion, created_at) values (:nombre, now() ) 
                    returning id;",
                    ["nombre" => $nombre]))->first();
                ...
                DB::select("UPDATE tbl_servicio set descripcion = :nombre , updated_at=now() where id = :id",
                    ["id" => $id, "nombre" => $nombre]);
                $msgSuccess = "Servicio ".$id." editado exitosamente.";
            }else if ($accion == 3) {
                DB::select("UPDATE tbl_servicio set deleted_at = now() where id = :id", ["id" => $id]);
                $msgSuccess = "Servicio ".$id." eliminado exitosamente.";
            }else{
                $msgError = "Acción inválida";
            }
                $servicio_list = collect(\DB::select("SELECT id, descripcion as nombre from tbl_servicio 
