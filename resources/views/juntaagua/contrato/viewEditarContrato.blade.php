        ...
        <div class="flex items-center justify-center flex-1 px-5 lg:justify-start">
            ...
            <div class="ml-5">
                <div class="text-lg font-medium truncate w-240 sm:w-80 sm:whitespace-normal">
                    ...
                    <h1 class="text-5xl font-medium leading-none">EDITAR CONTRATO</h1>
                </div>
                <div class="text-slate-500">Pantalla de edición de contratos.</div>
            </div>
        </div>
    </div>
</div>
<div class="p-5 mt-5 intro-y box">
    <div class="grid grid-cols-12 gap-6">
        ...
        @endif
        <form action="{{ route('update_contrato') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-2 gap-x-4">
                <div class="mb-4 ">
                    ...
                    <x-base.tom-select
                        id="id_cliente"
                        name="id_cliente"
                        class="w-full {{ $errors->has('id_cliente') ? 'border-red-500' : 'border-gray-300' }}"
                    >
                        <option value="">Seleccione un cliente</option>
                        @foreach ($clientes as $cliente)
                            <option value="{{ $cliente->id }}"
                                {{ old('id_cliente', $listarContrato->id_cliente) == $cliente->id ? 'selected' : '' }}>
                                {{ $cliente->cliente }}
                            </option>
                        @endforeach
                    </x-base.tom-select>
                ...
                <div class="mb-4">
                    ...
                    <select id="id_ubicacion" name="id_ubicacion" class="w-full p-2 border {{ $errors->has('id_ubicacion') ? 'border-red-500' : 'border-gray-300' }} rounded mt-1">
                        <option value="{{ $listarContrato->id_ubicacion }}" {{ old('id_ubicacion', $listarContrato->id_ubicacion) == $listarContrato->id_ubicacion ? 'selected' : '' }}>{{ $listarContrato->descripcion_casa }}</option>
                    </select>
                    @error('id_ubicacion')
                        <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <select id="id_servicio" name="id_servicio" class="w-full p-2 border {{ $errors->has('id_servicio') ? 'border-red-500' : 'border-gray-300' }} rounded mt-1">
                        ...
                        @foreach($servicios as $servicio)
                            <option value="{{ $servicio->id }}" {{ old('id_servicio', $listarContrato->id_servicio) == $servicio->id ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <input type="date" id="fecha_inicio" name="fecha_inicio" value="{{ old('fecha_inicio', $listarContrato->fecha_inicio) }}" class="w-full p-2 border {{ $errors->has('fecha_inicio') ? 'border-red-500' : 'border-gray-300' }} rounded mt-1">
                    @error('fecha_inicio')
                        <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                    @enderror
                </div>
                <div class="mb-4">
                    ...
                    <input type="date" id="fecha_fin" name="fecha_fin" value="{{ old('fecha_fin', $listarContrato->fecha_fin) }}" class="w-full p-2 border {{ $errors->has('fecha_fin') ? 'border-red-500' : 'border-gray-300' }} rounded mt-1">
                    @error('fecha_fin')
                        <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                    @enderror
                </div>
            </div>
            <button type="submit" class="w-full bg-primary text-white p-2 rounded mt-4">Guardar Cambios</button>
        </form>
...
@once
    ...
    <script type="module">
        ...
        //$('#fecha_inicio').val('{{ $listarContrato->fecha_inicio }}');
        //$('#fecha_fin').val('{{ $listarContrato->fecha_fin }}');
